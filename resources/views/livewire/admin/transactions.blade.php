<div>
    ...
    <main class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-sm font-medium text-muted-foreground">Réussies</h3>
            ...
            <div class="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-sm font-medium text-muted-foreground">Revenus</h3>
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-primary">
            ...
            <div class="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-sm font-medium text-muted-foreground">En attente</h3>
            ...
            <div class="flex flex-col space-y-1.5 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">{{ $transactions->total() }} transaction(s) enregistrée(s)</p>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        {{-- Statut --}}
                        <div>
                            <select wire:model.live="filterStatus" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-base ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm">
                        ...
                        {{-- Agrégateur --}}
                        <div>
                            <select wire:model.live="filterAgregateur" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-base ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm">
                    ...
                    <table class="w-full caption-bottom text-sm">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b transition-colors hover:bg-muted/50">
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground cursor-pointer" wire:click="sortBy('id')">
                                    ID
                                    @if($sortField === 'id')
                                ...
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground cursor-pointer" wire:click="sortBy('amount')">
                                    Montant
                                    @if($sortField === 'amount')
                                        <span class="text-xs">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Agrégateur</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Statut</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground cursor-pointer" wire:click="sortBy('created_at')">
                                    Date
                            ...
                            @forelse($transactions as $transaction)
                                <tr class="border-b transition-colors hover:bg-muted/50">
                                    <td class="p-4 align-middle">
                                        <span class="font-mono text-xs text-muted-foreground">#{{ $transaction->id }}</span>
                                    </td>
                                    <td class="p-4 align-middle">
                                        <div class="font-medium">{{ $transaction->fullname ?? 'N/A' }}</div>
                                        <div class="text-sm text-muted-foreground">{{ $transaction->email ?? $transaction->phone }}</div>
                                    </td>
                                    <td class="p-4 align-middle">
                                        <div class="font-semibold">{{ number_format($transaction->amount, 0, ',', ' ') }} {{ $transaction->currency }}</div>
                                        @if($transaction->fees)
                                            <div class="text-xs text-muted-foreground">Frais: {{ number_format($transaction->fees, 0, ',', ' ') }}</div>
                                        @endif
                                    </td>
                                    <td class="p-4 align-middle">
        ...
        <div class="fixed inset-0 z-[100] bg-black bg-opacity-80 flex items-center justify-center p-4" wire:click.self="closeModal">
            <div class="bg-white rounded-lg shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto relative">
                <div class="p-6">
                    ...
                    <div>
                        <label class="text-sm font-medium text-muted-foreground">Agrégateur</label>
                        <p class="mt-1 font-medium">{{ strtoupper($selectedTransaction->agregateur) }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-muted-foreground">Montant</label>
                        <p class="mt-1 text-2xl font-bold">{{ number_format($selectedTransaction->amount, 0, ',', ' ') }} {{ $selectedTransaction->currency }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-muted-foreground">Frais</label>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">Nom complet</label>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">Type de paiement</label>
                        ...
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">ID externe</label>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">Utilisateur</label>
                            <p class="mt-1">{{ $selectedTransaction->user->name }}</p>
                        </div>
                        @endif
                        @if($selectedTransaction->course)
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">Formation</label>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">Créée le</label>
                            <p class="mt-1">{{ $selectedTransaction->created_at->format('d/m/Y à H:i:s') }}</p>
                        </div>
                        @if($selectedTransaction->performed_at)
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">Exécutée le</label>
