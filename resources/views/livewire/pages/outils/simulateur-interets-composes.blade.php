    ...
    <section class="max-w-[1100px] mx-auto px-5 lg:px-8 py-14 lg:py-20">
                ...
                <div>
                    ...
                    <select wire:model.live="fcpType" class="w-full rounded-lg border border-[#c5c5d4] px-3 py-2.5 bg-[#f9f9ff] focus:border-[#001a61] outline-none">
                        @foreach ($availableTypes as $type)
                            <option value="{{ $type }}">FCP {{ $type }} — {{ number_format($rates[$type] ?? 0, 0) }} %</option>
                        @endforeach
                    </select>
                    <p class="mt-2 text-xs text-[#757683]">{{ $typeDescription }} Rendement indicatif : {{ number_format($typeRate, 0, ',', ' ') }} % / an.</p>
                </div>
                <div>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white border border-[#c5c5d4] rounded-xl p-5">
                    <div class="mb-3">
                        ...
                        <p class="text-xs text-[#757683]">Horizon : {{ $annees }} ans — {{ number_format($typeRate, 0) }} % / an</p>
                    </div>
        ...
        <div class="mt-8 bg-white border border-[#c5c5d4] rounded-xl p-5">
            ...
            <div class="space-y-3 text-sm text-[#444652]">
                <p>Le simulateur utilise des taux prévisionnels par classe d’actifs : <strong>FCP Actions 13 %</strong> et <strong>FCP Obligations 6 %</strong>.</p>
