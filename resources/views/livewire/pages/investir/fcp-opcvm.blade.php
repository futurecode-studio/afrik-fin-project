<div class="bg-[#f9f9ff] text-[#131c2a] min-h-screen">
    ...
    <section class="px-5 lg:px-16 max-w-[1280px] mx-auto pt-12 pb-6">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <p class="text-sm font-semibold tracking-widest uppercase text-[#0a2e8c] mb-3">OPCVM UEMOA</p>
                <h1 class="text-3xl md:text-4xl font-extrabold text-[#001a61]">FCP et OPCVM</h1>
                <p class="mt-2 text-[#444652]">Fonds suivis via Sikafinance — VL réelles, sans données fictives.</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('investir.vl') }}" class="text-sm font-bold text-[#001a61] border border-[#001a61] px-4 py-2 rounded hover:bg-[#e7eeff]">Valeurs liquidatives</a>
                <button type="button" wire:click="refresh" class="text-sm font-bold bg-[#001a61] text-white px-4 py-2 rounded hover:bg-[#0a2e8c]">
                    Actualiser
                </button>
            </div>
        </div>
    ...
    <section class="px-5 lg:px-16 max-w-[1280px] mx-auto pb-6">
        <div class="bg-white border border-[#c5c5d4] rounded-xl p-4 flex flex-col md:flex-row gap-3">
            <input type="search" wire:model.live.debounce.300ms="search" placeholder="Nom, société, ISIN…"
    ...
    <section class="px-5 lg:px-16 max-w-[1280px] mx-auto pb-20">
        @if ($loading)
            <p class="text-center py-16 text-[#757683]">Chargement des VL Sikafinance…</p>
        @elseif ($error)
                ...
                @forelse ($list as $f)
                    <a href="{{ route('investir.fcp-detail', $f['id']) }}"
                        ...
                        <div class="flex justify-between items-start gap-2">
                            ...
                            <span @class(['text-sm font-bold', 'text-green-600' => ($f['variation_percentage'] ?? 0) >= 0, 'text-red-600' => ($f['variation_percentage'] ?? 0) < 0])>
                                {{ $f['variation'] ?? '—' }}
                            </span>
                        </div>
                        <h2 class="font-bold text-[#001a61] mt-3 line-clamp-2">{{ $f['name'] }}</h2>
                        <p class="text-xs text-[#757683] mt-1">{{ $f['company'] }} · {{ $f['country'] }}</p>
                        <p class="text-xl font-extrabold text-[#131c2a] mt-4">{{ $f['nav_value'] }}</p>
                        <p class="text-xs text-[#757683] mt-1">VL au {{ \Carbon\Carbon::parse($f['date'])->format('d/m/Y') }}</p>
                    </a>
                @empty
                    <p class="col-span-full text-center py-16 text-[#757683]">Aucun fonds trouvé.</p>
                @endforelse
            </div>
        @endif
    </section>
